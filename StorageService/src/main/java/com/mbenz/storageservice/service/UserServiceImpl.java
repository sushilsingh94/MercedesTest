...
public class UserServiceImpl implements UserService{
    ...
    public HttpUserResponse getUserDetails(Integer id, String name, String email) throws Exception {
        ...
        return new HttpUserResponse(response.statusCode(), new ObjectMapper().readValue(AESUtil.decrypt(response.body()), UserResponse.class));
    }

    @Override
    public UserResponse saveUserDetails(UserRequest userRequest, FileType fileType) throws JsonProcessingException {
        log.info("Method Entry");
        ...
        kafKaProducerService.sendMessage(AESUtil.encrypt(new ObjectMapper().writeValueAsString(kafkaUserRequest)));
        return userResponse;
    }

    @Override
    public UserResponse updateUserDetails(Integer id, UserRequest userRequest, FileType fileType) throws JsonProcessingException {
        log.info("Method Entry");
        UserResponse userResponse = new UserResponse(userRequest.getName(), userRequest.getEmail(), userRequest.getSalary(), userRequest.getAge(), userRequest.getDob());
